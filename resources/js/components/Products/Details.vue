<template>
                ...
                <div class="row">
                    <div class="col-10 text-center">
                        <ul class="breadcrumb">
                            ...
                            <li><router-link to="/details" class="active">{{product.name}}</router-link></li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            <div class="row mx-auto">
                <div class="col-md-5">
                    <div class="img-wrapper">
                        <img :src="'img/products/' + product.image" class="card-img-top" alt="..." width="250px" style="max-width: 350px;"
                        @mouseover="imageMouseOver($event)" @mousemove="imageMouseMove($event)" @mouseout="imageMouseOut($event)">
                    </div>
                </div>
                <div class="col-md-7 text-left">
                    <div class="mx-2">
                        ...
                        <div class="pt-2">
                            ...
                            <input type="number" style="width:60px; border-radius: 5px; height: 35px; border:transparent; padding: 5px"
                            v-model="pQuantity" min="1" @change="productQuantity(product.id)" :max="product.amount">
                    <add-to-cart :pId="product.id" :pName="product.name" :pPrice="product.price" :pImage="product.image" :pAmount="product.amount" /><miniCart />
                        </div>
                        <miniList /><compareList />
                    </div>
                </div>
            </div>
        </div>
            ...
            <tab name="Reviews">
                <div class="heading text-left black">Reviews</div>
                    <div class="left-review text-left">
                        ...
                        v-model="totalRate" :increment="0.1" active-color="#ff9800" >
                        </star-rating>
                ...
                <div>
                    ...
                    <div class="orange font-weight-bold text-left page-header py-3 px-4">Customer Reviews ({{ totalComment }})</div>
                    <div class="row" v-for="comment in comments.data" :key="comment.id">
                        <div class="col-md-1 col-sm-2 hidden-xs">
                        <div class="thumbnail">
                            <div class="image">
                                <img src="img/profile/profile.png" class="img-circle elevation-1" alt="User Image" style="width: 55px">
                            </div>
                        ...
                        <div class="panel panel-default arrow left">
                            ...
                            <header class="text-left">
                                <div class="comment-date mt-3 mb-1" style="font-size: 12px"><i class="fas fa-clock mx-1"></i>{{ comment.created_at | myDate }}</div>
                                <div class="comment-user ml-2">
                                    <star-rating v-model="comment.rating" :increment="0.5" text-class="custom-text" 
                                        :read-only="true" :star-size="25">
                                    </star-rating>
                                </div>
                    ...
                    <pagination :data="comments" @pagination-change-page="getResults" class="justify-content-center my-2"></pagination>
                </div>
                <form class="d-block text-left mt-4" v-if="this.$gate.userId() > 0" @submit.prevent="setRating(product.id)">
                    <hr style="border:3px solid #f1f1f1">
                    <div class="text-center font-weight-bold p-3" style="background: #eee;">You're reviewing: {{product.name}}</div>
                    <h4 class="blue mt-3">Write your own review</h4>
                    <p v-if="(this.userRate === true)" class="red">You already reviewed this product.</p>
                    <h5 class="d-inline-block mb-3">Rating</h5>
                    <star-rating v-model="rating" :increment="0.5" text-class="custom-text"
                    :read-only="(this.userRate === true)" class="d-inline-block my-3" :star-size="25" min="1">
                    </star-rating>
                    <input type="price" name="price"
                            placeholder="Short description"
                            class="form-control mb-2" maxlength="15" v-model="form.short_description">
                    <has-error :form="form" field="short_description"></has-error>
                    <textarea name="" id="" class="form-control d-block" style="height: 170px" placeholder="Write your review"
                    v-model="form.comment" maxlength="550"></textarea>
                    <has-error :form="form" field="comment"></has-error>
                    <button  type="submit" class="btn btn-primary my-3">Publish</button>
                </form>
                <div v-if="this.$gate.userId() === null">
                    <hr style="border:3px solid #f1f1f1">
                    <div class="jumbotron">
                        <h5>You must be logged in to post a comment.</h5>
                        <router-link to="/login" class="d-block">Login</router-link>
                    </div>
                </div>
            </tab>
            <tab name="Details">
    ...
    <div class="newest py-5">
      <div class="container">
        ...
        <relatedProducts :products.sync="product" />
      </div>
    </div>
    ...
    methods: {
        ...
        loadProduct() {
            if(this.$route.params.p_product != undefined) {
                this.$store.commit('addLastProduct', this.$route.params.p_product);
                this.$store.commit('userActivities', this.$route.params.p_product);
            }
            this.product = this.$store.state.lastProduct;
        },
        ...
        getComment() {
            axios.get("api/details/"+this.product.id).then(({ data }) => (this.comments = data));  
        },
        getResults(page = 1) {
            axios.get('api/details?page=' + page)
            .then(response => {
            ...
            .then(res => {
                ...
                for(var i = 0; i < data.length; i++) {
                    ...
                    if(data[i]['user_id'] === uid) {
                        this.userRate = true;
                        this.rating = data[i]['rating'];
                    }

                    this.rateUser = false;

                    for (let j = i; j < data.length-1; j++) {
                        if(data[i]['user_id'] === data[j+1]['user_id']) {
                            this.rateUser = true;
                        }
                    }
        ...
        setRating(id) {
            ...
            deleteReview(id) {
                ...
                confirmButtonText: 'Yes, delete it!'
                    ...
                    if(result.value) {
                        this.form.delete('/api/details/'+id).then(() => {
                            swal.fire(
                                'Deleted!',
...
@media (min-width: 700px) {
  .tabs-component-panels, .product-info {
