          ...
          <div :class="(this.$store.state.cart.length === 0) ? 'col-md-10 mx-auto' : 'col-md-8'">
              <div class="card" style="max-height: 400px;">
                  ...
                  <div class="card-body table-responsive p-0">
                      <table class="table" v-if="this.$store.state.cart.length === 0">
                          <tr >
                      ...
                      <table class="table table-hover" v-if="this.$store.state.cart.length > 0">
                          ...
                          <tbody>
                              <tr v-for="item in this.$store.state.cart" :key="item.id">
                                <td height="70" width="100"><img :src="'img/products/' + item.image" class="card-img-top" alt="..." width="250px"></td>
                                <td>{{item.name}}</td>
                                <td>{{item.price | currency}}</td>
                                <td>
                                    <input type="number" style="width:60px; border-radius: 5px; height: 32px; border:transparent; padding: 5px"
                                    v-model="item.quantity" min="1" @change="productQuantity(item.id, item.quantity, item.amount)" :max="item.amount"></td>
                                <td>
                                    <button @click="$store.commit('removeFromCart', item)" class="btn btn-danger"><i class="fas fa-trash"></i></button>
                                </td>
                                </tr>
                          </tbody>
                      </table>
              ...
              <div class="card">
                  ...
                  <div class="card-body table-responsive p-0">
                      <table class="table">
                          ...
                          <tfoot>
                                <tr class="justify-content-between">
                                    <td><button class="btn btn-outline-dark btn-block p-2" @click="$store.commit('clearCart')">Clear Cart</button></td>
                                    <td><button class="btn btn-outline-primary btn-block p-2" @click="placeOrder()">Proceed to Checkout</button></td>
                                </tr>
                          </tfoot>
                      </table>
                  </div>
        ...
        methods: {
            ...
            placeOrder(event) {
                ...
                this.carts.forEach(product => {
                    ...
                    axios.post('api/orders/', {product_id: product.id, user_id: this.$gate.userId(), 
                    amount: product.quantity, total: product.price * product.quantity })
                    .then(data => {
                    }).catch(err => {
                        console.log(err);
                    ...
                    this.form.put('api/orders/'+ product.id)
                    .then(() => {
                        swal.fire(
                            'Updated!',
